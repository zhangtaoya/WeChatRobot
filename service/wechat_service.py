...
def new_user():
    ...
    '''
    doc = yield motordb.mongo_find_one(col, {'status': {'$in': [WECHAT_ACCOUNT_STATUS_INIT,
                                                                WECHAT_ACCOUNT_STATUS_WAIT_GEN_QR,
                                                                WECHAT_ACCOUNT_STATUS_WAIT_SCAN]},
                                             'ct': {'$gt': int(time.time() - 120)}})
    if doc is False:
